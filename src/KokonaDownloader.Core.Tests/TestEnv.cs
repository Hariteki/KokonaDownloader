...
internal static class TestEnv
{
    ...
    public sealed class FileServer : IDisposable
    {
        ...
        private readonly Dictionary<string, (byte[] Content, Dictionary<string, string> Headers, int ChunkBytes, int ChunkDelayMs)> _files = new();
        ...
        public void AddFile(string path, byte[] content, Dictionary<string, string>? headers = null,
            int chunkBytes = 0, int chunkDelayMs = 0)
            => _files[path.TrimStart('/')] = (content, headers ?? new Dictionary<string, string>(), chunkBytes, chunkDelayMs);
        ...
        private async Task HandleAsync(HttpListenerContext ctx)
        {
            try
            {
                ...
                else if (_files.TryGetValue(path, out var entry))
                {
                    ...
                    var range = TryParseRange(ctx.Request.Headers["Range"], entry.Content.Length);
                    if (range is { } r)
                    {
